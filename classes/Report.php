...
session_start();
$robotsContent = $_SESSION['robotsContent'];
$countHost = $_SESSION['countHost'];
$countSitemap = $_SESSION['countSitemap'];
$robotsSize = $_SESSION['robotsSize'];
$responseServerCode = $_SESSION['responseServerCode'];
...
if ($robotsSize < 32) {
    ...
    $sheet->setCellValue("E12", 'Размер файла robots.txt составляет ' . $robotsSize . ' кб, что находится в пределах допустимой нормы');
    ...
    $sheet->setCellValue("D12", 'Состояние');
    $sheet->setCellValue("D13", 'Рекомендации');
    $sheet->setCellValue("E12", 'Размера файла robots.txt составляет ' . $robotsSize . ' кб, что превышает допустимую норму');
...
if ($responseServerCode === 200) {
    ...
    $sheet->setCellValue("D18", 'Состояние');
    $sheet->setCellValue("D19", 'Рекомендации');
    $sheet->setCellValue("E18", 'При обращении к файлу robots.txt сервер возвращает код ответа ' . $responseServerCode);
...
$objectWriter = new PHPExcel_Writer_Excel5($xls);

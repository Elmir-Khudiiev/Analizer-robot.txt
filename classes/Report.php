...
$SearchHost = $_SESSION['SearchHost'];
$SearchSiteMap = $_SESSION['SearchSiteMap'];
$RobotSize = $_SESSION['RobotSize'];
...
if($RobotSize < 32) {
    ...
    $sheet->setCellValue("E12", 'Размер файла robots.txt составляет '.$RobotSize.' кб, что находится в пределах допустимой нормы');
    ...
    $sheet->setCellValue("D12", 'Состояние');
    $sheet->setCellValue("D13", 'Рекомендации');
    $sheet->setCellValue("E12", 'Размера файла robots.txt составляет '.$RobotSize.' кб, что превышает допустимую норму');
...
if($ResponseCode == 200) {
    ...
    $sheet->setCellValue("D18", 'Состояние');
    $sheet->setCellValue("D19", 'Рекомендации');
    $sheet->setCellValue("E18", 'При обращении к файлу robots.txt сервер возвращает код ответа '.$ResponseCode);
